function[fillhandle,msg]=jbfill(xpoints,upper,lower,color,edge,add,transparency,parent_axes)
...
if length(upper)==length(lower) && length(lower)==length(xpoints)
    ...
    if exist('parent_axes','var')
        fillhandle=fill(xpoints,filled,color,'parent',parent_axes);%plot the data
    else
